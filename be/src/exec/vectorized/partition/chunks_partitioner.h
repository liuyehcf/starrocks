...
public:
    ...
    Status accept(const std::function<bool(int32_t partition_idx, const ChunkPtr& chunk)>& consumer);
...
private:
    ...
    void split_chunk_by_partition(HashMapWithKey& hash_map_with_key, const ChunkPtr& chunk) {
        hash_map_with_key.append_chunk(chunk, _partition_columns);
    }

    // Fetch chunks from hash map, return true if reaches eos
    template <typename HashMapWithKey>
    bool fetch_chunks_from_hash_map(HashMapWithKey& hash_map_with_key,
                                    const std::function<bool(int32_t partition_idx, const ChunkPtr& chunk)>& consumer) {
        if (_hash_map_eos) {
            return true;
        }
    ...
    template <typename HashMapWithKey>
    bool fetch_chunks_from_null_key_value(
            HashMapWithKey& hash_map_with_key,
            const std::function<bool(int32_t partition_idx, const ChunkPtr& chunk)>& consumer) {
        if (_null_key_eos) {
            return true;
        }
