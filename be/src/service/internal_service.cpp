...
void PInternalServiceImplBase<T>::transmit_data(google::protobuf::RpcController* cntl_base,
                                                ...
                                                google::protobuf::Closure* original_done) {
    std::function<void()> func = [=]() {
        google::protobuf::Closure* done = original_done;
    ...
    Status st;
    while (!(st = _exec_env->brpc_pool1()->submit_func(func)).ok()) {
...
void PInternalServiceImplBase<T>::transmit_chunk(google::protobuf::RpcController* cntl_base,
    ...
    std::function<void()> func = [=]() {
        ...
        VLOG_ROW << "transmit data: " << (uint64_t)(request)
        ...
        VLOG_ROW << "transmit data: " << (uint64_t)(request)
                 << " fragment_instance_id=" << print_id(request->finst_id()) << " node=" << request->node_id()
                 << " cost time = " << MonotonicNanos() - begin_ts;
    };
    Status st;
    while (!(st = _exec_env->brpc_pool2()->submit_func(func)).ok()) {
        LOG(ERROR) << "submit failed";
    }
}
...
void PInternalServiceImplBase<T>::get_info(google::protobuf::RpcController* controller, const PProxyRequest* request,
                                           PProxyResult* response, google::protobuf::Closure* original_done) {
    std::function<void()> func = [=]() {
        google::protobuf::Closure* done = original_done;
        ClosureGuard closure_guard(done);
